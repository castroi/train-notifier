...
export const stationsById: Record<string, Station> = stations.reduce<Record<string, Station>>(
  (acc, station) => {
...
export function stationName(
  id: number | string,
  locale: 'english' | 'hebrew' = 'english',
): string | undefined {
  const station = stationsById[String(id)];
  if (!station) return undefined;
  return station[locale];
}

export function isKnownStation(id: number | string): boolean {
  return Object.hasOwn(stationsById, String(id));
}
